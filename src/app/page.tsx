...
export default function Home() {
  const [ useReading, setUseReading ] = useState(false);

  const handleReading = () => {
    setUseReading(!useReading);
  };
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-between p-24">
      ...
      <div className="flex flex-col items-center justify-center w-full h-full text-center">
        <button
          className={`flex items-center justify-center w-24 h-24 p-4 text-white ${useReading ? 'bg-green-500' : 'bg-red-500'} rounded-full shadow-lg focus:outline-none focus:ring-2 focus:ring-offset-2 ${useReading ? 'focus:ring-green-500' : 'focus:ring-red-500'} cursor-pointer`}
